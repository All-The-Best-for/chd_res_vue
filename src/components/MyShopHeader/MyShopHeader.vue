<template>
  <div class="shop-header">
      ...
      <!-- 大标题 -->
      <div class="shop-content" @click="changeShopShow" v-if="info.stallName">
        <!-- 商家图标 -->
        <img class="image" :src="info.stallIcon?ImageBasePathHeader+info.stallIcon:''">
        <div class="header-content">
          <h2 class="content-title">
            <span class="content-tag">
              <span class="mini-tag">品牌</span>
            </span>
            <span class="content-name">{{info.stallName}}</span>
            <i class="content-icon"></i>
          </h2>
          <div class="shop-message">
            <span class="shop-message-detail">评分{{info.stallStar}}</span>
              <span class="shop-message-detail">月售{{info.amount}}单</span>
              <span class="shop-message-detail">
                <!-- {{info.description}} -->
                {{info.stallText}}
              <span>约12分钟</span>
            </span>
            <!-- <span class="shop-message-detail">距离  暂无</span> -->
          </div>
        </div>
      </div>

      <!-- 展示的折扣信息及可展开按钮,使用v-if防止报错 -->
      <div class="shop-header-discounts" @click="changeSupportShow" v-if="info.supports">
        <div class="discounts-left">
          ...
          <div class="activity" :class="supportClasses[info.supports[0].type]">
            ...
            <span class="activity-content ellipsis">{{info.supports[0].content}}
            </span>
          </div>
        </div>
        ...
        <div class="shop-brief-modal" v-show="shopShow">
          <div class="brief-modal-content">
            ...
            <div class="mask-footer">
              <span class="iconfont icon-close" @click="changeShopShow"></span>
            </div>
          </div>
      ...
      <transition name="fade2">
        ...
        <div class="activity-sheet" v-show="supportShow">
          <div class="activity-sheet-content">
            ...
            <span class="iconfont icon-close" @click="changeSupportShow"></span>
            </div>
          </div>
...
export default {
    ...
    mounted () {
      this.$nextTick(()=>{
        this.getShopInfo({shops:this.shops})
      })
    },
    methods: {
      ...mapActions('myShopAbout',['getShopInfo']),
      changeShopShow(){
